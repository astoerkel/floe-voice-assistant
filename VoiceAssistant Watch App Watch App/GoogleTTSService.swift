...
class GoogleTTSService {
    ...
    private let apiKey: String = {
        ...
        // Try to load from build settings (Watch app specific)
        if let key = Bundle.main.object(forInfoDictionaryKey: "GOOGLE_TTS_API_KEY") as? String, !key.isEmpty {
            print("🔑 GoogleTTS: API key loaded from build settings (length: \(key.count))")
            return key
        }
        
        // Try to load from build settings with INFOPLIST_KEY prefix
        if let key = Bundle.main.object(forInfoDictionaryKey: "INFOPLIST_KEY_GOOGLE_TTS_API_KEY") as? String, !key.isEmpty {
            print("🔑 GoogleTTS: API key loaded from build settings with INFOPLIST_KEY prefix (length: \(key.count))")
            return key
        }
        
        // For Watch app development, use the same key as iPhone app
        let developmentKey = "AIzaSyC1qfvgVaXJKz5bRK3V0HzLSXz9VlTiJ3I"
        print("🔧 GoogleTTS: Using development API key for Watch app")
